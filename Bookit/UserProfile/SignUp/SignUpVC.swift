...
class SignUpVC: UIViewController , CLLocationManagerDelegate {
    ...
    @objc func acceptTandC() {
        let indexPath = IndexPath.init(row: 0, section: 0)
        let cell = self.tablView.cellForRow(at: indexPath) as! SignUpTableViewCell
        
        if (cell.btnTnC.tag == 1) {
            cell.btnSignUp.isUserInteractionEnabled = false
            cell.btnSignUp.layer.cornerRadius = 27.5
            cell.btnSignUp.clipsToBounds = true
            cell.btnSignUp.setTitle("SAVE & CONTINUE", for: .normal)
            cell.btnSignUp.backgroundColor =  .systemGray3
            cell.btnTnC.setImage(UIImage(named: "uncheck"), for: .normal)
            cell.btnTnC.tag = 0
        } else {
            ...
            cell.btnSignUp.clipsToBounds = true
            cell.btnSignUp.setTitle("SAVE & CONTINUE", for: .normal)
            cell.btnSignUp.backgroundColor =  BUTTON_DARK_APP_COLOR
            cell.btnTnC.setImage(UIImage(named: "checkMark"), for: .normal)
            cell.btnTnC.tag = 1
        }
    }
